	...
	<div id="m_list">
		...
		<c:if test="${count > 0}">
			...
			<c:if test="${startPage > 10}">
				<a href="/FORKI/content/MyPage/MessengerList.do?pageNum=${startPage - 10 }">[이전]</a>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="/FORKI/content/MyPage/MessengerList.do?pageNum=${i}">[${i}]</a>
			</c:forEach>

			<c:if test="${endPage < pageCount}">
				<a href="/FORKI/content/MyPage/MessengerList.do?pageNum=${startPage + 10}">[다음]</a>
			</c:if>
		</c:if>
	</div>
</div>