...
<c:if test="${count==0}">
			...
			<c:if test="${count>0}">
				<c:forEach var="article" items="${articleList}">
					<tr>
						<td width="100" align="center" bgcolor=""><c:out
								value="${number}" /> <c:set var="number" value="${number-1}" /></td>
						<td width="300" align="center" bgcolor=""><c:if
		...
		<p align="center">
			<c:if test="${count>0}">
				...
				<c:if test="${startPage > 10}">
					<a
						href="/FORKI/content/board/recommendList.do?pageNum=${startPage - 10 }">[이전]</a>
				</c:if>
				...
				<c:if test="${endPage < pageCount}">
					<a
						href="/FORKI/content/board/recommendList.do?pageNum=${startPage + 10}">[다음]</a>
				</c:if>
			</c:if>
		</p>
